...
**`mutations.md`** — add `truncate` as a proper section between `delete` and `returning`:

```markdown
## 4. `truncate` — Wipe an Entire Path

`truncate` removes all rows from a path in a single operation. It bypasses
the pipeline model entirely and requires no `where` clause — that is the
point. It is the explicit, intentional escape hatch for cases where you
genuinely need to empty a path.

```tql
truncate "store/orders"
```

Because `delete` without a `where` clause is a compile-time error, `truncate`
is the only way to wipe a full path. The separation is intentional — reaching
for `truncate` requires a conscious decision, not an accidental omission.

`truncate` does not support `returning`. If you need a receipt of the removed
rows, use a `delete` with an explicit `where true` — this is the one case
where a broad filter is acceptable.

```tql
// Capture all rows before wiping
delete from "store/orders" as o: commerce.Order
where true
returning {
    id:         o.id,
...
Like `delete`, rows are not physically removed immediately. The engine writes
tombstone markers to the `.wal` and memtable. Physical reclamation happens
during background LSM compaction.